  ...
  return (
    <section className="flex flex-col justify-start h-full object-contain">
      <div className="flex flex-row justify-between w-full ">
        ...
        { page == 0?
          <div className="backdrop-blur-xl overflow-auto no-scrollbar  p-6 w-full grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-2 gap-2 place-items-start rounded-box  justify-center">
        ...
        <div className="flex flex-row gap-2 w-full justify-center ">
        {
          page==0?[...Array(certificatePageSize)].map((val,index)=>{
            return <button key={index} onClick={()=>ChangePage(index)} className={`p-1 w-10 my-2 bg-black rounded-full text-white ${index==startCertificate/MAX_CERT_PAGE? "outline-white outline-1 outline " : ""} `}>{index+1}</button>
          })  : 
          [...Array(badgePageSize)].map((val,index)=>{
            return <button key={index} onClick={()=>ChangePage(index)} className={`p-1 w-10 my-2 bg-black rounded-full text-white ${index==startBadge/MAX_BADGES_PAGE? "outline-white outline-1 outline " : ""} `}>{index+1}</button>
          }) 
        }
